...
- 不能将 `_Borrow` 指针类型用作泛型类型参数
- 借用类型不支持 `_Trait` 实现；借用类型不支持成员函数
- 不能对包含借用成员的结构体取借用
...
- **Trait 向上转型**：需要显式转换；不允许从 trait 到具体类型的隐式向下转型
- **`T *_Borrow` → `void *_Borrow`**：当 `T` 是平凡数据类型（没有指针字段，不是 `_Owned struct`）时**隐式**允许；否则转换被**禁止**，即使使用显式转换也不行
  ```c
  struct S { int *ptr; };

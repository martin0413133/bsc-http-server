...
description: "BiSheng C 常见错误和修复。当你遇到 BSC 编译错误、需要调试代码或想避免所有权、借用、安全区域、traits 或泛型的常见陷阱时，使用此技能。"
---
...
### 5.1 可能为空的 `_Owned` 指针没有 `_Nullable`
```c
...
### 8.1 使用 valgrind 而非 gdb 来定位 double-free
...
### 8.3 在声称编译器 bug 之前检查去语法糖后的 AST（driver 模式）

大多数关于析构函数 double-free 的"编译器 bug"假设最终都是错误的。使用以下命令验证：
...
**不要为此使用 `clang -cc1 -fsyntax-only`**——它缺少系统包含，这会使每个 `_Owned struct` 在 AST 中虚假地变成"无效"。有关详细信息，请参见 `/bsc-compile` 技能 §6。

### 8.4 常见的库级 double-free 原因
...
- **手动的 `_Unsafe` 字节赋值覆盖了 `_Owned` 槽，而没有析构先前的内容**：旧的堆指针泄漏（单个副本）或别名（如果它们仅被 `memmove` 移位）。

### 8.5 排查流程总结

1. 在 valgrind 下重现 → 获取两个释放位置 + malloc 来源。
2. 在隔离环境中测试 → 确认/否认测试顺序污染。
3. Driver 模式 AST 转储 → 确认编译器析构函数插入正确。
4. 然后才考虑编译器级别的错误。实际上，>90% 的 BSC 项目中的 double-free 追溯到库级别名，而非代码生成。

### 8.6 已知编译器 bug：`if`/`while`/`for` 条件中的 `_Owned` 参数未被移动追踪

BSC 析构函数去语法糖阶段（`SemaBSCDestructor.cpp::VisitCompoundStmt`）仅对**外层**类型为 `BinaryOperator`、`DeclStmt` 或 `CallExpr` 的语句运行移动追踪。当消耗 `_Owned` 参数的函数调用出现在 `if`/`while`/`for`/`switch` **条件内部**时，外层语句是 `IfStmt`（等），因此移动追踪被完全跳过。`is_moved` 标志永远不会被设为 1，析构函数在作用域退出时会在已释放的内存上触发 → double-free。

**症状：** `free(): double free detected in tcache 2` 紧接在带有 `_Owned` 参数且包装在 `if` 条件中的函数调用之后。
...
int main(void) {
    Box b = Box::new();
    if (consume(b) == 1) { }  /* BUG：b 未被标记为已移动 → 在 } 处 double free */
    return 0;
}
```
...
- 有 bug 的模式（`if (consume(b) == 1) {}`）：没有 `b_is_moved = 1` 的赋值，只有初始的 `b_is_moved = 0` 和析构函数 `if (!b_is_moved)` 检查。

**解决方法**——在测试之前将调用结果存储到局部变量中：
...
- `lhs = call(owned);` 作为顶层赋值——BinaryOperator 被检查

**当你怀疑此 bug 时：** 将一个可疑调用点转换为解决方法形式。如果 double-free 消失，你就确认了。对同一作用域中的所有兄弟位置应用相同的解决方法。

### 8.7 已知编译器 bug：`_Owned` 局部变量的析构函数在借用它的返回表达式之前触发

析构函数去语法糖阶段将每个 `_Owned` 局部变量的析构函数 `IfStmt` 作为兄弟语句**在**作用域末尾的 `ReturnStmt` 之前发出。如果返回表达式是一个通过该局部变量的借用进行读取的调用（`return f(&_Const b, ...)`），则该调用运行在**已被析构的内存**上并返回错误的值。没有崩溃，没有警告——只是静默的损坏返回值。

这与 §8.6 不同：那是在 `if`/`while` 条件中缺少移动标志赋值。即使没有涉及移动，这也是析构函数插入和 `ReturnStmt` 的操作数求值之间的顺序错误。

**症状：** 一个包含 `_Owned` 局部变量和 `return f(...borrow of that local...);` 的 `_Safe` 函数返回 `_Bool` 或 `int`，但返回错误的值。在调用和返回之间添加任何 `_Unsafe { printf(...); }` 会使它"开始工作"——因为打印强制将值放入一个生命周期跨越析构函数的命名临时变量中。
...
├── IfStmt: if (!b_is_moved) ~String(b)     ← 析构函数在此触发
└── ReturnStmt
    └── CallExpr: a->equals(&_Const b)      ← 在析构函数之后读取 b
```

无 bug 模式的 `CompoundStmt`：
```
├── DeclStmt: b = make()
├── DeclStmt: b_is_moved = 0
...
同一组节点，不同的顺序。析构函数去语法糖阶段将析构函数 `IfStmt` 插入到语句列表的末尾，而不考虑尾随的 `ReturnStmt` 的操作数是否仍在借用拥有的局部变量。

**触发条件**——三个条件必须同时满足：
1. 函数在返回时有一个仍在作用域的 `_Owned` 局部变量（`String`、任何 `_Owned struct`）。
2. 返回表达式是一个以 `*_Borrow` 形式获取该局部变量的 `CallExpr`（直接或通过另一个参数间接）。
3. 被调用者实际**解引用**该借用以计算返回值。忽略借用参数的被调用者不会触发该 bug。

**解决方法**——首先将调用结果存入命名的局部变量：
...
存入局部变量强制将 `b` 的使用放入更早的 `DeclStmt`，该语句在析构函数槽之前完成。然后 `ReturnStmt` 只读取 `r`（普通的 `_Bool`，没有借用），因此析构函数顺序不再重要。

**如何在实际情况中注意到这个 bug：** 从辅助函数返回的值看起来反转或荒谬，但辅助函数自身的逻辑是正确的。在最后一次借用使用和返回之间插入 `printf`——如果 bug 消失，你就遇到了 §8.7。应用存入局部变量的解决方法。

**此 bug 适用于任何返回表达式形状，其中返回值依赖于被调用者通过范围内 `_Owned` 局部变量的借用进行读取**，包括：
- `return owned_local.method(...);` 其中方法读取 `this`
- `return helper(&_Const owned_local);`
- `return outer(inner(&_Const owned_local));`

**此 bug 在以下情况下不适用：**
- 返回表达式根本不引用 `_Owned` 局部变量
- 返回值已经是返回语句之前计算的普通副本
- `_Owned` 局部变量在返回前已被移出（然后析构函数被 `b_is_moved=1` 跳过，不存在已释放内存的读取）
...
| 从拥有的局部变量的借用返回错误值 | 先存入局部变量；参见 §8.7 |
